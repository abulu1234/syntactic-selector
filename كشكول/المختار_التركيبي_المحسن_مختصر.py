...
# ---------- استخراج التوكنات والمركبات مع السياقات المختصرة ----------
class TokenExtractor:
    WORD_RE = re.compile(r"^[\u0600-\u06FF]+$")
    def __init__(self, folder, stopwords, match_mode, max_contexts=5):
    ...
    def extract(self):
        ...
        contexts = defaultdict(list)  # حفظ السياقات النصية المختصرة
        ...
        for fn in os.listdir(self.folder):
            ...
            for i in range(len(toks)-1):
                ...
                # حفظ السياق النصي (مختصر)
                if len(contexts[key]) < self.max_contexts:
                    start = max(0, i-2)
...
# ---------- التحليل النهائي المحسن المختصر ----------
class BigramAnalysis:
    MIN_ENTROPY = 1.0
    def __init__(self, folder, stop_file, mode, out_excel, out_json, max_contexts=5):
        self.sw = StopWords(stop_file)
        self.extr = TokenExtractor(folder, self.sw, mode, max_contexts)
...
if __name__ == "__main__":
    BigramAnalysis(
        ...
        mode=2,
        out_excel="المركبات_المحسنة_مختصرة.xlsx",
        out_json="البيانات_التفاعلية_مختصرة.json",
        max_contexts=5  # حفظ 5 سياقات فقط لكل تركيب
    ).run()
